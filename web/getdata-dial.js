...
switch (Number(duration)) {
    case 86400000: showing="Showing last 24 hours."; break;
    case 172800000: showing="Showing last 48 hours."; break;
    case 604800000: showing="Showing last week."; break;
    case 1209600000: showing="Showing last 2 weeks."; break;
    case 2419200000: showing="Showing last monthh."; break;
    case 4838400000: showing="Showing last 2 months."; break;
    case 14515200000: showing="Showing last 6 months."; break;
    case 290304000012: showing="Showing last year."; break;
    default: showing="Showing last 48 hours.";
}
...
// var datumVal = new Date() - 86400000; // 24 hours
// var datumVal = new Date() -   172800000; // 48 hours
var datumVal = new Date() - Number(duration); // 48 hours
...
var paramsf = { 
                ...
                ExpressionAttributeValues: {
                  ":iottopic": { "S" : "sentryPi/freezer"},
                  ":datum": { "N" : datumVal.toString()}
                }
             };

// DynamoDB Query -  RPI Temp
var paramsr = { 
                TableName: 'sentrypi-data-capture',
                KeyConditionExpression: '#id = :iottopic and #ts >= :datum',
                ExpressionAttributeNames: {
...
var paramsbmp = {
                ...
                ExpressionAttributeValues: {
                  ":iottopic": { "S" : "sentryPi/bmp180"},
                  ":datum": { "N" : datumVal.toString()}
                }
             };

// DynamoDB Query - Motion Detection
var paramsmo = {
                TableName: 'sentrypi-data-capture',
                KeyConditionExpression: '#id = :iottopic and #ts >= :datum',
                ExpressionAttributeNames: {
...
var options2 = {
	...
	options: options2
      ...
      // build chart motion
      var ctxmo = document.getElementById('myChartmo').getContext('2d');
      var myChartmo = new Chart(ctxmo, {
        type: 'line',
        data: {
          // set placeholders
          labels: [],
...
function getData() {
  ...
  dynamodb.query(paramsr, function(err, data) {
    if (err) {
      console.log(err);
      return null;
    } else {
      ...
      // set the chart object data and label arrays
      myChartr.data.labels = labelValues;
      myChartr.data.datasets[0].data = temperatureValues;
      // redraw the graph canvas
      myChartr.update();
  } // else
 }); //dynamodb
...
// motion detection
  dynamodb.query(paramsmo, function(err, data) {
    if (err) {
      console.log(err);
      return null;
    } else {
      ...
      for (var i in data['Items']) {
        // read the values from the dynamodb JSON packet
        temperatureRead = parseFloat(data['Items'][i]['eventdata']['M']['motion']['N']);
        timeRead = new Date(data['Items'][i]['eventdata']['M']['datetime']['S']);
      ...
      // set the chart object data and label arrays
      myChartmo.data.labels = labelValues;
      myChartmo.data.datasets[0].data = temperatureValues;
      // redraw the graph canvas
      myChartmo.update();
  } // else
 }); //dynamodb
...
function getDialData() {
  ...
  // RPi
  dynamodb.query(dialparamsr, function(err, data) {
    if (err) {
      console.log(err);
      return null;
    } else {
      for (var i in data['Items']) {
        // read the values from the dynamodb JSON packet
        rpiTemp = parseFloat(data['Items'][i]['eventdata']['M']['temperature']['N'])*1.8+32;
 	//document.getElementById("rpiTemp").innerHTML = (rpiTemp.toFixed(1)).toString();
        document.getElementById("title-rpi").innerHTML = "SentryPI - RPi Temp: " + (rpiTemp.toFixed(1)).toString() + "&#8457;"; 
      } // for
  } // else
 }); //dynamodb
...
$(function() {
  ...
  setInterval(getDialData, 300000);
});
